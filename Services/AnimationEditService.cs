...
public class AnimationEditService
{
    ...
    public async Task<string?> AddText(string url, string text)
    {
        ...
        string firstLineArgs = $"drawtext=fontsize={fontSize}:line_spacing=4:font='Impact':text='{textInput.FirstLine}':fix_bounds=true:x=(w-text_w)/2:fontcolor=white:bordercolor=black:borderw=3";
        string secondLineArgs = $"drawtext=fontsize={fontSize}:line_spacing=4:font='Impact':text='{textInput.SecondLine}':fix_bounds=true:x=(w-text_w)/2:fontcolor=white:bordercolor=black:borderw=3";
